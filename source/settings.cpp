...
#define String(key, dflt)                             \
	...
	// Procedural - Island
	section("ProceduralIsland");
	Int(PROCEDURAL_MAP_WIDTH, 256);
	Int(PROCEDURAL_MAP_HEIGHT, 256);
	Float(PROCEDURAL_ISLAND_SIZE, 0.80f);
	Float(PROCEDURAL_ISLAND_FALLOFF, 2.0f);
	Float(PROCEDURAL_ISLAND_THRESHOLD, 0.30f);
	Float(PROCEDURAL_NOISE_SCALE, 0.01f);
	Int(PROCEDURAL_NOISE_OCTAVES, 4);
	Float(PROCEDURAL_NOISE_PERSISTENCE, 0.5f);
	Float(PROCEDURAL_NOISE_LACUNARITY, 2.0f);
	Int(PROCEDURAL_CLEANUP_ENABLED, 1);
	Int(PROCEDURAL_MIN_LAND_PATCH, 4);
	Int(PROCEDURAL_MAX_WATER_HOLE, 3);
	Int(PROCEDURAL_SMOOTHING_PASSES, 2);
	Int(PROCEDURAL_WATER_ID, 4608);
	Int(PROCEDURAL_GROUND_ID, 4526);

	// Procedural - Dungeon
	section("ProceduralDungeon");
	Int(PROCEDURAL_DNG_WIDTH, 128);
	Int(PROCEDURAL_DNG_HEIGHT, 128);
	Int(PROCEDURAL_DNG_WALL_ID, 1030);
	Int(PROCEDURAL_DNG_FLOOR_ID, 406);
	Int(PROCEDURAL_DNG_ROOM_COUNT, 15);
	Int(PROCEDURAL_DNG_MIN_ROOM_SIZE, 5);
	Int(PROCEDURAL_DNG_MAX_ROOM_SIZE, 12);
	Int(PROCEDURAL_DNG_CORRIDOR_WIDTH, 2);
	Int(PROCEDURAL_DNG_GENERATE_CAVES, 1);
